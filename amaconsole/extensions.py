...
def load_extension_from_script(script_path: Path, verbose:bool = True) -> List[cmd2.CommandSet]:
    ...
    try:
        ...
        if ext != '.py':
            raise Exception("Script {script_path} isn't a python script")
        ...
        for name, cls in inspect.getmembers(module):
            if issubclass(cls, cmd2.CommandSet) and cls != cmd2.CommandSet:
                if verbose:
...
def load_extensions(basedir: Path, verbose:bool = True) -> List[cmd2.CommandSet]:
    ...
    for filename in os.listdir(basedir):
        if filename.endswith('.py'):
            ...
            extensions += load_extension_from_script(filepath)
