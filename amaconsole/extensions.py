...
def load_extensions_from_script(script_path: Path, verbose:bool = True) -> List[cmd2.CommandSet]:
        ...
        for name, cls in inspect.getmembers(module):
            try:
                if cls and issubclass(cls, cmd2.CommandSet) and cls != cmd2.CommandSet:
                    if verbose:
                        print(f"Importing {name} extension from {script_path}")
...
def load_extensions(basedir: Path, verbose : bool = True) -> List[cmd2.CommandSet]:
    ...
    for filename in os.listdir(basedir):
        if filename.endswith('.py'):
            ...
            extensions += load_extensions_from_script(filepath, verbose)
